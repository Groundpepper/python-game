...
class Area:
    ...
    def __init__(self, alignment, coordinates=None, terrain=None, name=None):
        ...
        while unit != "START":
            ...
            if board[int(row) - 1][int(column) - 1 + none_num].alignment == "Poland":
                if units_p2[int(unit) - 1].size <= board[int(row) - 1][int(column) - 1 + none_num].capacity:
                    ...
                    units_p2[int(unit) - 1].coordinates = [row, column]
            ...
            while units_poland[number].coordinates is None:
                ...
                if board[row][column].alignment == "Poland":
                    if units_poland[number].size <= board[row][column].capacity:
                        ...
                        units_poland[number].coordinates = [row + 1, column + 1]
...
def play_game(units_p1, units_p2):
    ...
    while 0 == 0:
        ...
        while choice == "1":
            ...
            for number in range(len(units_p1)):
                ...
                print("Where would you like to move {}?".format(units_p1[number].name))
                print()
    ...
    # may need to link board items to each other via linked-list style, established at board_setup()
